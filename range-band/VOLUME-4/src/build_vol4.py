...
from vol4_spec_a import DEEP_HEALTH  # noqa: E402
from vol4_spec_b import LIFE_RANGE  # noqa: E402
...
from widgets4 import draw_capacity, draw_ground, draw_blotter  # noqa: E402

# new series accents (patched at runtime; frozen brand.py untouched)
brand.ACCENT["DEEP HEALTH SERIES"] = (0.24, 0.38, 0.45)
brand.ACCENT["LIFE RANGE SERIES"] = (0.60, 0.34, 0.36)

VOL4 = DEEP_HEALTH + LIFE_RANGE
for _t in VOL4:
    _t.setdefault("price", 9.99)
OUT_ROOT = Path("/home/user/new-catalog-v4")
...
WIDGETS = {
    ...
    "capacity": draw_capacity, "ground": draw_ground, "blotter": draw_blotter,
}
...
def main():
    ...
    text_issues = []
    rows = []
    for i, t in enumerate(sorted(VOL4, key=lambda x: int(x["n"])), 1):
        t0 = time.time()
        row = build_one(t)
        # keyword uniqueness
        for k in t["keywords"]:
    ...
    with open(OUT_ROOT / "METADATA_VOL4.csv", "w", newline="", encoding="utf-8") as f:
