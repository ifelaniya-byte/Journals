...
class CoreViolationError(Exception):
    ...
    def __init__(self, message: str, violation_type: str = "unknown", context: Optional[Dict[str, Any]] = None):
        """
        Initialize CoreViolationError with enhanced context.
        
        Args:
            message: Error message
            violation_type: Type of violation (e.g., "resource_ceiling", "safety_boundary")
...
class OmegaCore:
    ...
    def check_resource(self, name: str, usage: float) -> bool:
        ...
        if usage > ceiling.limit:
            raise CoreViolationError(
                ...
                violation_type="resource_ceiling",
                context={
                    "resource_name": name,
                    "requested_usage": usage,
                    "ceiling_limit": ceiling.limit,
                    "unit": ceiling.unit,
                    "exceeded_by": usage - ceiling.limit
                }
            )
        return True
    ...
    def assert_boundary(self, boundary_name: str) -> None:
        ...
        if boundary_name not in names:
            raise CoreViolationError(
                f"Unknown safety boundary: {boundary_name}",
                violation_type="unknown_boundary",
                context={
                    "requested_boundary": boundary_name,
                    "available_boundaries": list(names)
                }
            )
